...
async function main() {
  ...
  const DbiliaTokenContract_new = await DbiliaToken.deploy("Dbilia New", "NFT", 25);
  ...
  console.log("New DbiliaToken contract - address: ", DbiliaTokenContract_new.address);
  ...
  for (let i=1; i<=totalSupply_current; i++) {
    ...
    tx = await DbiliaTokenContract_new.mintForDataMigration(tokenOwner, tokenURI, i);
    console.log("mintForDataMigration - tx: ", tx.hash);

    // replicate royalty receiver of current DbiliaToken contract to the new one
    const [percentage, receiverId] = await DbiliaTokenContract_current.royaltyReceivers(i);
    console.log("percentage: ", +percentage.toString(), ", receiverId: ", receiverId);
    tx = await DbiliaTokenContract_new.setRoyaltyReceiver(i, receiverId, percentage);
    console.log("setRoyaltyReceiver - tx: ", tx.hash);

    // replicate token owner of current DbiliaToken contract to the new one
    const [isW3user, w3owner, w2owner] = await DbiliaTokenContract_current.tokenOwners(i);
    console.log("isW3user: ", isW3user, ", w3owner: ", w3owner, ", w2owner: ", w2owner);
    try {
      tx = await DbiliaTokenContract_new.setTokenOwner(i, isW3user, w3owner, w2owner);
    ...
    for (const productIdEdition of productIdEditionList) {
      ...
      const tokenId = await DbiliaTokenContract_current.productEditions(productId, edition);
      tx = await DbiliaTokenContract_new.setProductEditionTokenId(tokenId, productId, edition);
